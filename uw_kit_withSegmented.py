...
def plot_con_green_by_floor(
    ...
    ax.set_ylabel('Mean window-view green ratio')
    ax.set_title(
        'Pheonix green ratio by building height and form'
    )
    ax.set_xticks(
...
def plot_houston_green_by_floors(
    df,
    ax=None,
    min_count=5,
    show_counts=False
    ...
    d['n_floors'] = pd.to_numeric(
        d['n_floors'],
        errors='coerce'
    )
    d['green'] = pd.to_numeric(
        d['green'],
        errors='coerce'
    )
    # Remove invalid records
    d = d.dropna(
        subset=['n_floors', 'green', 'rise_class']
    ).copy()
    d = d[
        (d['n_floors'] >= 1) &
        (d['green'].between(0, 1))
    ].copy()
    d['n_floors'] = d['n_floors'].round().astype(int)
    styles = [
        ('low-rise', '#2c7fb8'),
        ('mid-rise', '#31a354'),
        ('high-rise', '#d95f0e')
    ...
    for rise_class, color in styles:
        ...
        if s.empty:
            continue
        # Summarize by actual total building floors
        summary = (
            s.groupby('n_floors', as_index=False)
            .agg(
                mean_green=('green', 'mean'),
                std_green=('green', 'std'),
                n_buildings=('green', 'size')
            )
            .sort_values('n_floors')
        )
        # Keep floor counts supported by enough buildings
        summary = summary[
            summary['n_buildings'] >= min_count
        ].copy()
        if summary.empty:
            continue
        summary['std_green'] = (
            summary['std_green'].fillna(0)
        )
        # Shift each building class so its first valid floor count is x = 1
        first_floor = summary['n_floors'].min()
        summary['relative_floor_position'] = (
            summary['n_floors'] - first_floor + 1
        )
        lower = (
            summary['mean_green'] -
            summary['std_green']
        ...
        plotted_max = max(
            plotted_max,
            int(summary['relative_floor_position'].max())
        )
        ax.fill_between(
            summary['relative_floor_position'].to_numpy(),
            lower.to_numpy(),
            upper.to_numpy(),
            color=color,
            alpha=0.20,
            linewidth=0,
            label=f'{rise_class} (±1 SD)'
        )
        ax.plot(
            summary['relative_floor_position'],
            summary['mean_green'],
        ...
        if show_counts:
            for _, row in summary.iterrows():
                ax.annotate(
                    (
                        f"n={int(row['n_buildings'])}\n"
                        f"{int(row['n_floors'])} floors"
                    ),
                    (
                        row['relative_floor_position'],
                        row['mean_green']
                    ),
                    xytext=(0, 8),
                    textcoords='offset points',
    ...
    ax.set_xlabel(
        'Relative building-height position within each building form'
    )
    ax.set_ylabel('Mean building-level green ratio')
    ax.set_title(
        'Houston green ratio by relative building height and form'
    )
    # Avoid labeling every value when the range is very long
    tick_step = max(1, int(np.ceil(plotted_max / 15)))
    ax.set_xticks(
        range(1, plotted_max + 1, tick_step)
    )
    ax.set_xlim(0.5, plotted_max + 0.5)
    ax.set_ylim(bottom=0)
    ax.grid(alpha=0.2)
    ax.legend(title='Building form')
    return ax


def plot_houston_lst_vs_green_by_building_form(
    df,
    lst_col='lst',
    green_col='green',
    floor_col='n_floors',
    ...
    for col in [lst_col, green_col, floor_col]:
        d[col] = pd.to_numeric(
            d[col],
            errors='coerce'
        )
    ...
    for ax, (form, display_name) in zip(
        axes,
        forms
    ):
        form_data = d[
        ...
        for floor_bin in labs:
            ...
            ax.scatter(
                ...
                s=20,
                alpha=0.55,
                color=floor_colors[floor_bin],
                label=floor_bin
            )

        r = form_data[green_col].corr(
            form_data[lst_col]
        )
    ...
    # Construct a complete legend even when a category
    # is absent from the high-rise panel
    ...
    legend_handles = [
        Line2D(
            ...
            linestyle='none',
            markersize=7,
            markerfacecolor=floor_colors[label],
    ...
    fig.suptitle(
        'Houston land-surface temperature versus '
        'green ratio by building form'
    )
...
def plot_houston_lst_vs_green(
    ...
    d[green_col] = pd.to_numeric(
        d[green_col],
        errors='coerce'
    )
    d[lst_col] = pd.to_numeric(
        d[lst_col],
        errors='coerce'
    )

    d = d.dropna(
        subset=['rise_class', green_col, lst_col]
    )

    for form, color in styles:
        s = d[d['rise_class'] == form]

        if s.empty:
            continue

        r = s[green_col].corr(s[lst_col])
        ...
        ax.scatter(
            ...
            alpha=0.5,
            color=color,
            label=f'{form}: r={r:.2f}, n={len(s):,}'
        )

    ax.set_xlabel('Building green ratio')
    ax.set_ylabel('Land-surface temperature (°C)')
    ax.set_title(
        'Houston surface temperature versus green ratio'
    )
    ax.grid(alpha=0.2)
    ax.legend(title='Building form')
...
def plot_con_green_by_floor(
    ...
    ax.set_title(
        'Pheonix window-view green by floor and building form'
    )
    ax.set_xticks(
        range(1, plotted_max_floor + 1)
    )
